...
import FblNumGrid from './fblNumGrid'

function FreeBusinessListNum ({ freeblnumData }) {
  let dataItem = freeblnumData.list.list
  ...
  if(dataItem != "" && dataItem != null && dataItem.length !=0){
    console.log(freeblnumData.list.list)
  ...
  return (
    <div >
      <FblNumGrid dataSource={res}/>
    </div>
  )
}

FreeBusinessListNum.propTypes = {
  freeblnumData: PropTypes.object,
}

export default connect(({ freeblnumData }) => ({ freeblnumData }))(FreeBusinessListNum)
