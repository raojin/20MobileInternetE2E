...
import FblGBGrid from './fblGBGrid'


function FreeBusinessListGB ({ freeblGbData }) {
  let dataItem = freeblGbData.list.list
  ...
  if(dataItem != "" && dataItem != null && dataItem.length !=0){
    console.log(freeblGbData.list.list)
  ...
  return (
    <div >
      <FblGBGrid dataSource={res}/>
    </div>
  )
}

FreeBusinessListGB.propTypes = {
  freeblGbData: PropTypes.object,
}

export default connect(({ freeblGbData }) => ({ freeblGbData }))(FreeBusinessListGB)
