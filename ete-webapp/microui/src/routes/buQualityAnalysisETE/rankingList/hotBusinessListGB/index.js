...
import HblGBGrid from './hblGBGrid'

function HotBusinessListGB ({ hotblgbData }) {
  let dataItem = hotblgbData.list.list
  ...
  if(dataItem != "" && dataItem != null && dataItem.length !=0){
    console.log(hotblgbData.list.list)
  ...
  return (
    <div >
      <HblGBGrid dataSource={res}/>
    </div>
  )
}

HotBusinessListGB.propTypes = {
  hotblgbData: PropTypes.object,
}

export default connect(({ hotblgbData }) => ({ hotblgbData }))(HotBusinessListGB)
